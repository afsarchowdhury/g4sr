...
#' @examples
#' gfs_clean_attendance_student_session(2020, "2020-07-01")
#' gfs_clean_attendance_student_session(2020, "2020-07-01", "am")
#' @export
gfs_clean_attendance_student_session <- function(academicYear, goDate, session = NULL) {
  ## Message
  message(cat(crayon::cyan("Generating clean student session marks for", goDate)))

  ## Import data
  df_attendance_codes <- gfs_attendance_codes(academicYear)
  df_attendance_student_session_marks <- gfs_attendance_student_session_marks(academicYear, goDate)
...
#' @param goDate date as string in the form yyyy-mm-dd.
#' @examples
#' gfs_clean_attendance_student_lesson(2020, "2020-07-01")
#' @export
gfs_clean_attendance_student_lesson <- function(academicYear, goDate) {
  ## Message
  message(cat(crayon::cyan("Generating clean student lesson marks for", goDate)))

  ## Import data
  df_attendance_codes <- gfs_attendance_codes(academicYear)
  df_attendance_student_lesson_marks <- gfs_attendance_student_lesson_marks(academicYear = academicYear, goDate)
  df_students <- gfs_student_details(academicYear)
  df_students_details <- gfs_student_edu_details(academicYear)
  df_classes <- gfs_classes(academicYear)
  df_timetables <- gfs_timetables(academicYear)
...
#' @param academicYear academic year as integer.
#' @examples
#' gfs_clean_attendance_student_summary(2020)
#' @export
gfs_clean_attendance_student_summary <- function(academicYear) {
  ## Message
  message(cat(crayon::cyan("Generating clean student attendance summary")))

  ## Import data
  df_attendance_codes <- gfs_attendance_codes(academicYear)
  df_attendance_student_summary <- gfs_attendance_student_summary(academicYear)
  df_students <- gfs_student_details(academicYear)
  df_students_details <- gfs_student_edu_details(academicYear)
...
#' @param session session as string.
#' @examples
#' gfs_clean_attendance_student_session_range(2021, "2021-07-12", "2021-07-16")
#' gfs_clean_attendance_student_session_range(2021, "2021-07-12", "2021-07-16", session = "am")
#' @export
gfs_clean_attendance_student_session_range <- function(academicYear, goDateStart, goDateEnd, session = NULL) {
  ## Message
  ...
  df_attendance_student_session_marks <- lapply(1:length(goDate),
                                                function(i) gfs_attendance_student_session_marks(academicYear = academicYear,
                                                                                                 goDate = goDate[i]))
  # Bind list
...
#' @param goDateEnd end date of range as string in the form yyyy-mm-dd.
#' @examples
#' gfs_clean_attendance_student_lesson_range(2021, "2021-07-12", "2021-07-16")
#' @export
gfs_clean_attendance_student_lesson_range <- function(academicYear, goDateStart, goDateEnd) {
  ## Message
  ...
  df_attendance_student_lesson_marks <- lapply(1:length(goDate),
                                               function(i) gfs_attendance_student_lesson_marks(academicYear = academicYear,
                                                                                               goDate = goDate[i]))
  # Bind list
